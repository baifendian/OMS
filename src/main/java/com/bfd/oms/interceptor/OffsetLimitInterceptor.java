...
public class OffsetLimitInterceptor implements Interceptor {
	...
	public Object intercept(final Invocation invocation) throws Throwable {
		...
		final PageBounds pageBounds = PageUtil.createPageBounds(parameter);
		// 获取执行方法 并对方法判读 不是page 的直接执行
		String idStr = ms.getId();
		int id = idStr.lastIndexOf(".");
		String methodName = idStr.substring(id + 1);
		// 判断是否为 分页方法
		if (!methodName.toUpperCase().contains("PAGE")) {
